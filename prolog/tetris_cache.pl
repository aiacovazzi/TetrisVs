:- module(tetris, [startGbL/1,getPathOfBestMove/2,writeGameBoard/0,placePiece/3,start/1,tetraminos/1,nextNodes/3,evaluateNode/2,takeMove/4,evaluateMovement/2,rotate/2,left/2,right/2,down/2]).
...
callPlacePiece(BranchId,Tetraminos,GbList,[(T,R,C)|Taill],[[Tetraminos,GbListPost,ClRow,(T,R,C)]|Tail2]):-
    checkElegibility(T,R,C,GbList),
    placePiece(T,R,C,GbList,GbListPost,ClRow),
    %save computed branch for recycle
    rotation(Tb, T, _),
    assertz(savedBranch(BranchId,[Tb,GbList],[GbListPost,ClRow,(T,R,C)])),
    %write('saved'),write(savedBranch(Level,[Tb,GbList],[GbListPost,ClRow,(T,R,C)])),nl,
    callPlacePiece(BranchId,Tetraminos,GbList,Taill,Tail2),
    !.

callPlacePiece(BranchId,Tetraminos,GbList,[_|Taill],Tail2):-
    callPlacePiece(BranchId,Tetraminos,GbList,Taill,Tail2).
...
retractIfFirstLevel(0,SavedId) :-
     retractIfFirstLevel(0,0,SavedId),
     %write('deleted '),write(SavedId),nl,
     !.

retractIfFirstLevel(_,_).

retractIfFirstLevel(0,SavedId,SavedId) :-
    retractall(savedBranch(SavedId,_,_)),
    !.

retractIfFirstLevel(0,Mid,SavedId) :-
    retractall(savedBranch(Mid,_,_)),
    Mid1 is Mid + 1,
    retractIfFirstLevel(0,Mid1,SavedId).
...
nextNodes(Level,Node, NextNodes) :-
    ...
    %write('looking for '),write([Tb,GbL]),nl,
    bagof(([Tetraminos,GbListPost,ClRow,(T,R,C)]),(savedBranch(_,[Tb,GbL],[GbListPost,ClRow,(T,R,C)])),NextNodes),
    %write(' found it'),nl,
    savedBranch(SavedId,[Tb,GbL],_),
    retractIfFirstLevel(Level,SavedId),
    %write('OK'),nl,
    !.
...
nextNodes(Level,Node, NextNodes) :-
    ...
    num(Id),
    BranchId is Id + Level,
    callPlacePiece(BranchId,Tetraminos,GbL,L,NextNodes),
    !.
